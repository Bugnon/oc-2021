...
def retour():
    ...
    for l in state:
        for xres in l:
            ...
            caseback = Case(correspondance[(yres, xres)], state[yres][xres])
        yres += 1
    global hist
    hist.pop()
    global score
...
def changement(ypos, xpos, ysuiv, xsuiv):
    print(ypos, xpos, ysuiv, xsuiv)
    global state
    global correspondance
    if state[ypos][xpos] != state[ysuiv][xsuiv]:
        if state[ysuiv][xsuiv] == 0:
            nbrsuiv = state[ypos][xpos]
        else:
            nbrsuiv = state[ypos][xpos] * 2
        xcoordsuiv, ycoordsuiv = correspondance[ysuiv, xsuiv]
        casei = Case((xcoordsuiv, ycoordsuiv), nbrsuiv)
        state[ysuiv][xsuiv] = nbrsuiv
        xcoordpos, ycoordpos = correspondance[ypos, xpos]
        case0 = Case((xcoordpos, ycoordpos), 0)
        state[ypos][xpos] = 0
        global nbr
        nbr -= 1
        global modifi
...
def operation(xres, yres, direction):
    global correspondance
    xcoord, ycoord = correspondance[(yres, xres)]
    xmemoire, ymemoire = xcoord, ycoord
    if direction == 'h':
        ycoord += 87.5
    elif direction == 'b':
        ycoord -= 87.5
        ...
        xcoord -= 87.5
    coord_possible = []
    for coord in correspondance:
        coord_possible.append(correspondance[coord])
    if (xcoord, ycoord) in coord_possible:
        global correspondance_inverse
        ysuiv, xsuiv = correspondance_inverse[(xcoord, ycoord)]
        return ysuiv, xsuiv
    else:
        global not_op
        not_op = 1
        ysuiv, xsuiv = correspondance_inverse[(xmemoire, ymemoire)]
        return ysuiv, xsuiv
...
def operation_inverse(xsuiv, ysuiv, direction):
    global correspondance
    xcoord, ycoord = correspondance[(ysuiv, xsuiv)]
    if direction == 'h':
        ycoord -= 87.5
    elif direction == 'b':
        ycoord += 87.5
...
def notsame(ypos, xpos, direction, ysuiv, xsuiv):
    global state
    global correspondance
    global not_op
    fusible = 0
    while True:
        if state[ysuiv][xsuiv] == 0:
            ysuiv, xsuiv = operation(xsuiv, ysuiv, direction)
            if not not_op:
                fusible += 1
                if fusible > 5:
                    break
                ...
                not_op = 0
                changement(ypos, xpos, ysuiv, xsuiv)
                break
        elif state[ypos][xpos] == state[ysuiv][xsuiv]:
            changement(ypos, xpos, ysuiv, xsuiv)
            break
        elif state[ypos][xpos] != state[ysuiv][xsuiv]:
            ysuiv, ysuiv = operation_inverse(xsuiv, ysuiv, direction)
            changement(ypos, xpos, ysuiv, xsuiv)
            break
...
def calcul(pos, direction):
    ...
    global correspondance
    ypos, xpos = pos
    ysuiv, xsuiv = operation(xpos, ypos, direction)
    if not not_op:
        if state[ypos][xpos] == state[ysuiv][xsuiv]:
            changement(ypos, xpos, ysuiv, xsuiv)
        else:
            notsame(ypos, xpos, direction, ysuiv, xsuiv)
    else:
        not_op = 0
    ...
    if endjeu:
        ...
        for i in range(4):
            for res in correspondance:
                yres, xres = res
                if state[yres][xres] != 0:
                    calcul(res, direction)
        if modifi == 1:         
            new()
            resultat()
        else:
